...
taula de la regla I (com a avís); «adaptat»; que cada igualtat escrita estigui bé, calculada sencera amb l'ordre de les operacions («2 + 3 · 4 = 14», «(2 + 3) · 4 = 20», «3² = 3 · 3 = 9», «√16 = 4»), també al solucionari. Una igualtat falsa a posta, com l'error típic que descriu un solucionari, va ratllada (`.ratllat`) i no es comprova. Les que l'alumnat ha de revisar (i que poden ser falses a posta) van dins de `.revisa`, i tampoc no es comproven: el solucionari en diu les bones; que no hi hagi cap DOCX d'examen dins del repositori, fora de `docx/`, perquè tot es publica al web; que el text manuscrit només porti caràcters que Caveat té;
